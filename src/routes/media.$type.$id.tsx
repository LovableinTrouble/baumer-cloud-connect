...
export const Route = createFileRoute("/media/$type/$id")({
  head: () => ({ meta: [{ title: "Title Details — Baumer" }] }),
  component: MediaPage,
});
...
function MediaPage() {
  ...
  const handleShare = async () => {
    ...
    const title = media?.title ?? "Watch on Baumer";
    if (navigator.share) {
      try {
              ...
              <div className="flex max-w-full flex-col gap-2 pt-1.5 sm:flex-row sm:flex-wrap sm:items-center">
                <Link
                  ...
                  onClick={() => stashWatchMedia(media)}
                   className="liquid-pill play-button-hover group/btn relative z-10 inline-flex h-12 w-full shrink-0 items-center justify-center gap-2.5 rounded-xl px-6 text-[15px] font-bold sm:mr-1 sm:w-auto sm:px-7"
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="h-4 w-4 fill-current transition-transform duration-500 ease-out group-hover/btn:translate-x-0.5"
                  >
                ...
                <div className="-my-3 flex w-full items-center gap-2 overflow-x-auto overflow-y-visible no-scrollbar py-3 sm:w-auto sm:overflow-visible">
                <button
                  type="button"
