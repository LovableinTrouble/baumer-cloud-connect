  ...
  head: () => ({
    meta: [
      { title: "Explore — Baumer" },
      {
        name: "description",
        content:
          "Browse movies, TV series, and anime in one place. Filter by genre, year, and rating.",
      },
    ],
  }),
...
function ExplorePage() {
    ...
    queryFn: async () => {
      ...
      if (providerId != null) {
        ...
        if (type === "movie" || type === "all") {
          results.push(
            ...(genreId != null
              ? await fetchByGenre("movie", genreId, 2)
        ...
        if (type === "tv" || type === "all") {
          results.push(
            ...(genreId != null ? await fetchByGenre("tv", genreId, 2) : await fetchTrending("tv")),
          );
          if (type === "all" && genreId == null) results.push(...(await fetchPopular("tv", 1)));
        }
        if (type === "anime") {
          results.push(
            ...(genreId != null ? await fetchAnimeByGenre(genreId, 2) : await fetchAnime(3)),
          );
        }
        if (type === "upcoming") {
  ...
  const activeService =
    providerId != null ? (STREAMING_SERVICES.find((s) => s.id === providerId) ?? null) : null;
...
function ProviderButton({
  ...
  active: { name: string; logo: string; accent: string } | null;
  onOpen: () => void;
  onClear: () => void;
}) {
  return (
    <div className="liquid-glass flex h-11 min-w-0 items-center rounded-full text-xs font-semibold transition">
      ...
      >
        <span
          className="grid h-6 w-6 shrink-0 place-items-center overflow-hidden rounded-lg bg-white/8 ring-1 ring-white/12"
          style={active ? { background: active.accent } : undefined}
        >
          {active ? (
...
function SortSelect({ value, onChange }: { value: SortKey; onChange: (v: SortKey) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div
        className={`absolute right-0 top-12 z-40 w-40 overflow-hidden rounded-2xl border border-white/10 bg-[oklch(0.16_0.02_280)] p-1.5 text-white shadow-2xl transition duration-150 ${open ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-1 opacity-0"}`}
      >
...
function ProviderPicker({
  ...
  const filtered = useMemo(() => {
    ...
    return STREAMING_SERVICES.filter(
      (s) => s.name.toLowerCase().includes(term) || s.blurb.toLowerCase().includes(term),
    );
  }, [q]);
    ...
    <div className="fixed inset-0 z-[9999] grid h-dvh w-dvw place-items-center overflow-hidden bg-black/72 px-4 py-6 backdrop-blur-md animate-fade-in">
      ...
      <div className="relative z-10 flex h-[min(620px,calc(100dvh-48px))] w-full max-w-[430px] flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[oklch(0.15_0.02_280)] text-white shadow-[0_28px_90px_rgba(0,0,0,0.62)] animate-soft-rise">
        ...
        <div className="mt-3 flex-1 space-y-1 overflow-y-auto px-3 pb-4 pt-1">
          {filtered.length === 0 && (
            <div className="grid place-items-center py-10 text-sm text-white/55">
              No services match "{q}"
            </div>
          )}
          {filtered.map((s, i) => (
